...
struct klmFilter
{
    ...
    cv::Mat meas = cv::Mat::zeros(6, 1, CV_32F);
    cv::KalmanFilter kf;
    bool found = false;
    int notFoundCount = 0;
    ...
    int lastMeasureX=0, lastMeasureY = 0;
...
struct kcf
{
    ...
    kcf(const cv::Rect & b, const cv::Mat & frame, const int & nf):bbox(b)
    {
        locations.push_back(b);
        numFrame.push_back(nf);
...
public:
     ...
     */
    inline void addNewTrackers(const std::vector<dt> & posDetections, std::vector<kcf> * trackerList, const cv::Mat & frame, const int & nframe);
     ...
     */
    inline void nms(std::vector<dt> * posDetections);
     ...
     */
    inline void roiSelection(const std::vector<cv::RotatedRect> & rectangles, std::vector<cv::Mat> * regions, std::vector<cv::Rect2d> * boundingLocations,
                      const cv::Mat & frame);
     ...
     */
    inline void resize(std::vector<cv::Mat> * regions);
     ...
     */
    inline klmFilter initFilter(const cv::Rect & boundingBox, const int & nbFrame);
     ...
     */
    void updateTrackers(const std::vector<dt> & posDetections, std::vector<kcf> * trackerList, const cv::Mat & frame, const int & nframe,
                               const cv::Ptr<cv::ml::SVM> & svm, std::vector<kcf> * significantTrackers);

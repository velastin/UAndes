...
void BgsubTrack::addNewTrackers(const vector<dt> & posDetections, vector<kcf> * trackerList, const Mat & frame, const int & nframe)
{
    for(int i=0; i < posDetections.size(); i--)
    {
        ...
        if(trackerList->size() == 0)
        {
            ...
            trackerList->push_back(kcf(posDetections[0].boundingBox, frame, nframe));
        }
        else
        {
            bool flag_nms = false;
            ...
            if(! flag_nms)
            {
                ...
                trackerList->push_back(kcf(posDetections[i].boundingBox, frame, nframe));
            }
        }
    }
}
...
void BgsubTrack::updateTrackers(const vector<dt> & posDetections, vector<kcf> * trackerList, const Mat & frame, const int & nframe,
                                const Ptr<ml::SVM> & svm, vector<kcf> * significantTrackers)
{
    for(int i=trackerList->size() -1; i >= 0; i--)
    {
        ...
        for(int j=0; j < posDetections.size(); j++)
        {
            ...
            if(overlap_area / (float) total_area > 0.5) // BOSS : 0.5 / subway : 0.1 - 0.2
            {
                ...
                {*/
                    //cout << "updated from measure " << endl;
                    ...
                    (*trackerList)[i].locations.push_back((*trackerList)[i].bbox);
        ...
        // if no overlapping with detections has been found this can be due to missing detection or new person. Here we try to recover from target loss
        if(! flag)
        {
            ...
            if((*trackerList)[i].locations.size() >= 2)
            {
                ...
                //only needed for kcf tracker since prediction is sometimes far out of the image bounds
                if(predict_x > frame.cols || predict_y > frame.rows)
                {
                ...
                if(roi_x + width > frame.cols)
                    width = 96 - (roi_x+96 - frame.cols +1);
                if(roi_y < 0)
                    roi_y = 0;
                if(roi_y + height > frame.rows)
                    height = 96 - (roi_y+96 - frame.rows +1);
                ...
                if(redetections.size() > 0)
                {
                    ...
                    {*/
                        //cout << "update from redetection " << endl;
                        //imshow("redetection", frame(redetections[0].boundingBox).clone());
                        ...
                        // reinitialize the tracker with the redetected target. Assuming we did not consider another target
                        (*trackerList)[i].tracker->init(frame, redetections[0].boundingBox);
...
int main( int argc, char** argv )
{
    ...
    // Trackers
    vector<kcf> trackerList;
    vector<kcf> significantTrackers;
    vector<vector<Rect2d> > trackedRegions;
...
//==========================
        ...
        {
            Rect2d bbox;
            trackerList[i].tracker->update(frame, bbox);
            if(! (bbox.width == 0 && bbox.height == 0))
                trackerList[i].bbox = bbox;
        }
